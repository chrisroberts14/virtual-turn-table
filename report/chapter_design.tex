...
\subsection{React}
React is a JavaScript library for creating interactive web interfaces. Its modular component based system


React is a library for creating interactive web interfaces. The interface is split into modular components that can be reused reducing code duplication. As it is widely used there are many different component libraries in this project I used HeroUI TODO: REF HERE. This sped up development immensely as time was not wasted on developing styling of low level components such as buttons and was instead spent implementing the features of the application.

TODO: Add detail about React

TODO: Add detail about advantages

\subsection{Docker}
Docker is a platform that allows the containerisation of applications. This means they can run in their own environments and then communication ports can then be exposed so they can take in and send out messages. The benefit here is that any device running Docker can run the application by building the Dockerfile which is invaluable when it comes to deployment as the code and Dockerfile can be provided to a cloud provider who can then build and run the application autonomously.
